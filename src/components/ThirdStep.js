  ...
  useEffect(() => {
    const getCountries = async () => {
      try {
        ...
        let allCountries = [];
        allCountries = result?.map(({ isoCode, name }) => ({ // ES11 optional chaining operator ?, the code after the ? will be executed only if the [revious reference is neither undefined nor null
          isoCode,
          name
        }));
